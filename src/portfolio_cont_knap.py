...
def portfolio_bilevel(nom_return, return_dev, hedge_cost, covariance, max_variance, k, b, w, f, file_name, Gamma=2, gam=0.2):
    ...
    # set objective
    m.setObjective(gp.quicksum(nom_return[i] * y[i] for i in assets)
    ...
    # dual lower level
    m.addConstrs(pi * f[a] + lam[a] >= return_dev[a] * y[a] for a in assets)

    # primal lower level
    m.addConstr(gp.quicksum(f[a] * u[a] for a in assets) <= b + gp.quicksum(w[a] * s[a] for a in assets))

    # strong duality
    m.addConstr(gp.quicksum(lam[a] for a in assets) + (pi * (b + (gp.quicksum(w[a] * s[a] for a in assets)))) <= gp.quicksum(y[a] * return_dev[a] * u[a] for a in assets))
...
def portfolio_robust(nom_return, return_dev, hedge_cost, covariance, max_variance, k, b, w, f, file_name, Gamma=2, gam=0.2):
    ...
    # set objective
    m.setObjective(gp.quicksum(nom_return[i] * y[i] for i in assets)
                    - gp.quicksum(lam[a] for a in assets) - (pi * (b + (gp.quicksum(w[a] * s[a] for a in assets)))), GRB.MAXIMIZE)
...
def solve_instance_bilevel(file_name):
    ...
    b, w, f = parse_ll_knapsack(file_name.replace(".po", ".kp"))

    portfolio_bilevel(nom_return, return_dev, hedge_cost, covariance, max_variance, k, b, w, f, file_name)
...
def solve_instance_robust(file_name):
    ...
    portfolio_robust(nom_return, return_dev, hedge_cost, covariance, max_variance, k, b, w, f, file_name)
...
def parse_ll_knapsack(file_path):
    """Parses the .kp file and returns the knapsack parameters
    ...
    w : list
        list of right-hand side weights
    f : list
    ...
    # Extract right-hand side weights
    w_block = lines[3].strip()
    w = [float(value) for value in w_block.strip('[]').split()]

    # Extract item weights
    f_block = lines[5].strip()
    f = [float(value) for value in f_block.strip('[]').split()]

    print("Knapsack capacity:", b)
    print("Right-hand side weights:", w)
    print("Item weights:", f)
    return b, w, f