...
## Description
This repository contains the open-source implementations accompanying the paper "A Computational Study for Solving Decision-Dependent Robust Problems as Bilevel Optimization Problems" by [Henri Lefebvre](https://henrilefebvre.com/), [Martin Schmidt](https://martinschmidt.squarespace.com/), [Simon Stevens](https://simstevens.github.io/) and [Johannes Thürauf](https://www.johannesthuerauf.com/).
...
`--uncertainty`
The kind of uncertainty that is to be chosen, e.g. 'cont_budgeted', 'cont_knapsack', 'discrete_budgeted', or 'discrete_knapsack'.
...
The argument `--yasol_type` has to be added to specify the type of `.qlp` formulation that should be used. The choices are `bilevel` and `existeval`.  
